...
Rectangle {
    ...
    Timer {
        id: typewriterTimer
        interval: 30
        running: timeDateRoot.moduleActive && timeDateRoot.showDate && barWindow && barWindow.isStartupReady && typeInIndex < fullDateStr.length
        repeat: true
    ...
    MouseArea {
        ...
        onClicked: {
            timeDateRoot.showDate = !timeDateRoot.showDate;
            if (timeDateRoot.showDate) timeDateRoot.typeInIndex = 0;
        }
    }
    ...
    Rectangle {
        ...
        radius: barWindow ? barWindow.s(timeDateRoot.isCompact ? 9 : 10) : (timeDateRoot.isCompact ? 9 : 10)
        color: timeDateRoot.isGrouped ? "transparent" : (timeDateRoot.isSolid ? (timeDateRoot.distinctPills ? (timeDateRoot.isHovered ? ThemeBackend.surface0 : "#110915") : "transparent") : (timeDateRoot.isHovered ? ThemeBackend.surface0 : ThemeBackend.base))
        border.width: 0
        visible: height > 0
    ...
    Item {
        ...
        Column {
            ...
            Text {
                ...
                color: timeDateRoot.isCompact ? Qt.lighter("#F0E3B6", 1.1) : "#F0E3B6"
            }
            Text {
                anchors.left: parent.left
                visible: timeDateRoot.showDate
                text: dateStr
                font.family: ThemeBackend.fontFamily
