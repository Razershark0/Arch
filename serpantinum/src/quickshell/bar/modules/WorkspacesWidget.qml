...
Rectangle {
    ...
    radius: barWindow ? barWindow.s(workspacesWidgetRoot.isCompact ? 9 : 10) : (workspacesWidgetRoot.isCompact ? 9 : 10)
    border.width: 0
    ...
    Rectangle {
        ...
        function isOccupiedAt(i) {
            if (workspacesWidgetRoot.isNiri) return !!workspacesWidgetRoot.niriOccupiedMap[i];
            if (workspacesWidgetRoot.isSway) return !!workspacesWidgetRoot.swayOccupiedMap[i];
            let ws = workspacesWidgetRoot.wsForId(i + 1);
        ...
        function getX(index, activeIndex) {
            ...
            for (let i = 0; i < index; i++) {
                // Hidden (unoccupied, non-active) dots take zero space in the
                // Row, so they must not contribute to the offset either.
                if (i !== activeIndex && !isOccupiedAt(i)) continue;
