...
Variants {
    ...
    delegate: Component {
        PanelWindow {
            ...
            property real currentBarMinY: verticalOffset
            property real currentBarMaxY: verticalOffset + effectiveBarHeight
            ...
            property real baseOffsetY: {
                if (barPosition === "bottom") {
                    ...
                    return isFill ? (cornerRadius / 3 - 1) : edgePadding;
                }
            }
            ...
            exclusiveZone: (!barConfigReady || autohide) ? 0 : barHeight
            color: "transparent"

            property real activeMaskHeight: (autohide && !isRevealed) ? s(4) : (isVertical ? (isFill ? barWindow.height : (effectiveBarHeight + edgePadding * 2)) : (isFill ? (barHeight + cornerRadius) : (barHeight + edgePadding)))
            property real activeMaskWidth: (autohide && !isRevealed) ? s(4) : (isVertical ? (isFill ? (barHeight + cornerRadius) : (barHeight + edgePadding)) : (isFill ? barWindow.width : (effectiveBarWidth + edgePadding * 2)))
            ...
            Binding { target: NotifPanelController; property: "panelWidth"; value: barWindow.systemPillWidth }
            Binding { target: NotifPanelController; property: "topOffset"; value: barWindow.pillBottomY }
