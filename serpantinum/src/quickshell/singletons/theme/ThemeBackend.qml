pragma Singleton
import QtQuick
import Quickshell
import Quickshell.Io
import "../../"

Item {
    id: root

    property string fontFamily: "Adwaita Mono"
    property int borderRadius: 8

    property color base: "#1e1e2e"
    property color mantle: "#181825"
    property color crust: "#11111b"
    property color text: "#cdd6f4"
    property color subtext0: "#a6adc8"
    property color subtext1: "#bac2de"
    property color surface0: "#313244"
    property color surface1: "#45475a"
    property color surface2: "#585b70"
    property color overlay0: "#6c7086"
    property color overlay1: "#7f849c"
    property color overlay2: "#9399b2"
    property color blue: "#89b4fa"
    property color sapphire: "#74c7ec"
    property color peach: "#fab387"
    property color green: "#a6e3a1"
    property color red: "#f38ba8"
    property color mauve: "#cba6f7"
    property color pink: "#f5c2e7"
    property color yellow: "#f9e2af"
    property color maroon: "#eba0ac"
    property color teal: "#94e2d5"

    function resolveHex(val) {
        if (!val) return "";
        if (typeof val === "string") return val;
        if (typeof val === "object" && val.hex) return val.hex;
        return val.toString();
    }

    function applyColorObject(c) {
        if (!c) return;
        const keys = ["base", "mantle", "crust", "surface0", "surface1", "surface2",
            "overlay0", "overlay1", "overlay2", "text", "subtext0", "subtext1",
            "blue", "sapphire", "peach", "green", "red", "mauve", "pink", "yellow",
            "maroon", "teal"];
        for (let i = 0; i < keys.length; i++) {
            let hex = resolveHex(c[keys[i]]);
            if (hex) root[keys[i]] = Qt.color(hex);
        }
    }

    function updateFromConfig() {
        let themeConfig = Config.getSetting("theme", {});
        if (themeConfig.fontFamily !== undefined) root.fontFamily = themeConfig.fontFamily;
        if (themeConfig.borderRadius !== undefined) root.borderRadius = themeConfig.borderRadius;
        if (themeConfig.colors) root.applyColorObject(themeConfig.colors);
    }

    Connections {
        target: Config
        function onSettingsLoaded() { root.updateFromConfig(); }
    }

    Component.onCompleted: root.updateFromConfig()
}
